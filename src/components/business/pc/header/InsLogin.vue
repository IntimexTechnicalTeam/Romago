<template>
  <div class="handle-one" v-click-outside="closeDialog">
    ...
    <div class="top-member-detail top-window" v-if="isShow && !isLogin">
      ...
      <div class="facebook_login" @click="fbLogin" v-if="!isIe && $Settings.siteVersion !== 1">
  ...
  checkEmail (rule, value, callback) {
   ...
   getAccount () {
    ...
    this.$Api.member.getAccount().then(result => {
       if (result.Succeeded) {
          if (result.ReturnValue.Logined) {
            _this.$store.dispatch('setUser', (result.ReturnValue.FirstName + ' ' + result.ReturnValue.LastName).toUpperCase());
            _this.$store.dispatch('doLogin');
          } else {
